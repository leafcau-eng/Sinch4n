...
export default function Footer() {
  return (
    <footer className="relative z-10 w-full border-t border-white/10 bg-[#0a0a0a] px-6 py-10">
      <div className="mx-auto flex max-w-5xl flex-col items-center gap-6 text-center">
        <div className="flex items-center justify-center gap-4 flex-wrap">
          {SOCIALS.map((s) => (
            <a
              ...
              aria-label={s.name}
              className="w-10 h-10 flex items-center justify-center rounded-full border border-white/10 text-neutral-400 transition-all hover:text-cyan-400 hover:border-cyan-400/50 hover:-translate-y-1"
            >
        ...
        <p className="font-mono text-[11px] uppercase tracking-widest text-neutral-600">
          © {new Date().getFullYear()} SCH · Rian Riyandi
        </p>
      </div>
    </footer>
  );
